...
const VuraApp = () => {
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gray-50 p-6 font-sans">
      <header className="mb-8 w-full max-w-6xl flex flex-col md:flex-row justify-between items-center md:items-end border-b pb-4 gap-4">
        <div className="text-center md:text-left">
             <h1 className="text-4xl font-black text-gray-900 tracking-tighter cursor-pointer" onClick={() => setHasStarted(false)}>VURA<span className="text-blue-600">.</span></h1>
             <p className="text-gray-500 font-medium text-sm mt-1">Spatial Design Engine</p>
        </div>
        <div className="flex gap-2 w-full md:w-auto justify-center">
            <input 
                className="bg-transparent text-center md:text-right font-bold text-gray-700 outline-none border-b border-transparent focus:border-blue-500 hover:border-gray-300 transition w-full md:w-auto"
                value={roomName}
                onChange={(e) => setRoomName(e.target.value)}
            />
            <div className="relative">
              <button onClick={() => setShowHistory(!showHistory)} className="p-2 hover:bg-gray-200 rounded-full transition text-gray-500" title="History">
                <LayoutGrid size={20}/>
              </button>
              {showHistory && (
                <div className="absolute top-10 right-0 bg-white shadow-xl rounded-xl p-2 w-64 z-50 border border-gray-100">
                  <h3 className="text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider px-2">Recent Rooms</h3>
                  {recentRooms.length === 0 && <p className="text-sm text-gray-400 px-2">No saved rooms</p>}
                  {recentRooms.map(r => (
                    <button key={r.id} onClick={() => loadRoom(r)} className="text-left w-full px-2 py-2 hover:bg-gray-50 rounded-lg text-sm text-gray-700 truncate">
                      {r.name}
                    </button>
                  ))}
                </div>
            ...
            <button onClick={saveRoom} className="p-2 hover:bg-gray-200 rounded-full transition text-blue-600" title="Save"><Save size={20}/></button>
        </div>
      </header>

      {/* Action Bar */}
      <div className="flex flex-col md:flex-row flex-wrap justify-between items-center w-full max-w-6xl mb-6 bg-white p-4 rounded-2xl shadow-sm border border-gray-100 gap-4">
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto justify-center items-center">
             <label className="flex items-center gap-2 bg-black text-white px-5 py-2.5 rounded-xl cursor-pointer hover:bg-gray-800 transition text-sm font-bold w-full sm:w-auto justify-center">
        ...
        <div className="flex flex-col xl:flex-row gap-4 border-t md:border-t-0 md:border-l pt-4 md:pt-0 pl-0 md:pl-4 border-gray-200 w-full md:w-auto items-center">
             <div className="flex flex-wrap justify-center gap-2">
                 <button onClick={() => setIsCalibrating(true)} className="flex items-center gap-2 px-4 py-2 hover:bg-blue-50 text-blue-700 rounded-lg transition font-medium text-sm border md:border-none border-blue-100">
                     <Ruler size={16} /> {ppi > 0 ? `${ppi.toFixed(1)} px/in` : 'Calibrate'}
                 </button>
                 
                 {/* Layout Controls */}
                 <div className="flex flex-wrap bg-gray-100 rounded-lg p-1 gap-1">
                    <button onClick={autoArrange} className="px-3 py-1 bg-white shadow-sm rounded-md text-gray-700 font-medium text-xs hover:bg-gray-50 grow text-center">Line</button>
                    <button onClick={applyGrid} className="px-3 py-1 hover:bg-white hover:shadow-sm rounded-md text-gray-600 font-medium text-xs transition grow text-center">Grid</button>
                    <button onClick={applyMosaic} className="px-3 py-1 hover:bg-white hover:shadow-sm rounded-md text-gray-600 font-medium text-xs transition grow text-center">Mosaic</button>
                    <div className="w-px bg-gray-300 mx-1 hidden sm:block"></div>
                    <button onClick={apply57InchRule} className="px-3 py-1 hover:bg-blue-50 hover:text-blue-700 rounded-md text-gray-600 font-medium text-xs transition grow text-center" title="Center at 57 inches">57" Rule</button>
                 </div>
                 
                 {/* Nudge Controls */}
                 <div className="flex bg-gray-100 rounded-lg p-1 gap-1">
                    <button onClick={() => nudge(0, -10)} className="p-1 hover:bg-white rounded-md text-gray-600 w-8 flex justify-center" title="Nudge Up">↑</button>
                    <button onClick={() => nudge(0, 10)} className="p-1 hover:bg-white rounded-md text-gray-600 w-8 flex justify-center" title="Nudge Down">↓</button>
                    <button onClick={() => nudge(-10, 0)} className="p-1 hover:bg-white rounded-md text-gray-600 w-8 flex justify-center" title="Nudge Left">←</button>
                    <button onClick={() => nudge(10, 0)} className="p-1 hover:bg-white rounded-md text-gray-600 w-8 flex justify-center" title="Nudge Right">→</button>
                 </div>
             </div>

             <button onClick={downloadGuide} className="flex items-center gap-2 px-4 py-2 bg-blue-100 hover:bg-blue-200 text-blue-800 rounded-lg transition font-bold text-sm w-full md:w-auto justify-center">
      ...
      <div 
        ...
        className="relative w-full max-w-6xl bg-gray-200 rounded-3xl overflow-hidden shadow-2xl border-4 border-white aspect-video group"
      >
        {!wallImage ? (
